...
psrsim::psrsim () :
  Pulsar::Application ("psrsim", "pulsar modeling program")
#if HAVE_PGPLOT
  , plot (false)
#endif
{
  has_manual = false;
  version = "$Id: psrsim.C,v 1.13 2010/05/28 21:56:32 straten Exp $";
...
void psrsim::add_options (CommandLine::Menu& menu)
{
...
#if HAVE_PGPLOT
  arg = menu.add (this, &psrsim::set_plot_result, 'd');
  ...
  arg = menu.add (this, &psrsim::set_plot_options, 'c', "cfg[s]");
  ...
  arg = menu.add (this, &psrsim::add_exclude, "exclude", "deg0:deg1");
  arg->set_help ("add a range of pulse longitude to exclude from fit");

  arg = menu.add (this, &psrsim::add_include, "include", "deg0:deg1");
  arg->set_help ("add a range of pulse longitude to include in fit");

  arg = menu.add (this, &psrsim::add_opm, 'o', "deg0:deg1");
  ...
  arg = menu.add (this, &psrsim::map_chisq, 'x');
...
void psrsim::map_chisq()
{
  rvmfit->set_chisq_map (true);
}


void psrsim::add_include (const std::string& arg)
{
  range incl = range_deg_to_rad (arg);
  cerr << "psrsim: include " << arg << " degrees" << endl;

  rvmfit->add_include( incl );
}

void psrsim::add_exclude (const std::string& arg)
{
  range excl = range_deg_to_rad (arg);
  cerr << "psrsim: exclude " << arg << " degrees" << endl;

  rvmfit->add_exclude( excl);
}

void psrsim::add_opm (const std::string& arg)
{
  range opm = range_deg_to_rad (arg);
  cerr << "psrsim: OPM at " << arg << " degrees" << endl;

  rvmfit->add_opm( opm );
}

#if HAVE_PGPLOT
void psrsim::set_plot_options (const std::string& arg)
{
  vector<string> opts;
  separate(arg, opts, ",");
...
{
  if (!fit_rvm)
    throw Error (InvalidState, "psrsim",
  ...
  if (!global_search.empty())
  {
    ...
    cerr << "psrsim: search " << nalpha << "X" << nzeta << " grid" << endl;
  }
}
...
{
  if (verbose)
    cerr << "psrsim: fitting " << data->get_filename() << endl;
  ...
  if (nalpha && nzeta)
  {
    cerr << "psrsim: performing search of chi^2 map" << endl;

    rvmfit->global_search (nalpha, nzeta);
  }
  else
  {
    cerr << "psrsim: solving with initial guess: \n"
...
void psrsim::output_residuals ()
{
  if (output_QU_residuals)
  {
    string filename = "psrsim_QU_residuals.txt";
    ofstream out (filename.c_str());
    if (!out)
      throw Error (FailedSys, "psrsim", "could not open " + filename);
    else
  ...
  {
    string filename = "psrsim_psi_residuals.txt";
    ofstream out (filename.c_str());
    if (!out)
      throw Error (FailedSys, "psrsim", "could not open " + filename);
    else

...
#include "Pulsar/Integration_utils.h"
#include "Pulsar/PolnProfile.h"
#include "Pulsar/Profile_utils.h"
...
pa_plot( int rise_bin, int fall_bin,
	 ...
	 float nsigma, const vector<unsigned>& add_chans,
	 const vector<vector<unsigned> >& plot_add_chans);

vector<double>
get_per_bin(int rise_bin,int fall_bin,
	    Reference::To<Pulsar::Archive> data,
	    unsigned ichan);
...
double
get_stddev(int on_pulse,double mean,
	   const vector<double>& per_bin);

vector<double>
get_offsets(Reference::To<Pulsar::Archive> copy);

void
do_singlebin(Reference::To<Pulsar::Archive> data,
	     const vector<int> goodchans, bool display,
	     vector<double> freqs, float iphs,
	     vector<double>& pa, vector<double>& pa_stddev,
	     float nsigma, const vector<unsigned>& add_chans,
	     const vector<vector<unsigned> >& plot_add_chans);
...
void
do_window_offset(Reference::To<Pulsar::Archive> data,
		 float x1, float x2, bool display,
		 const vector<int> goodchans,
		 vector<double> freqs,
		 bool correct,
...
int main (int argc, char** argv)
{
  ...
  for (unsigned i = 0; i < archives.size(); i++) try
  {
    ...
    if (singlebin)
      do_singlebin    (data,goodchans,display, freqs, iphs,
		       pa, pa_stddev, nsigma, add_chans,
		       plot_add_chans);

    else if (window)
      do_window       (data,x1,x2,display, goodchans, freqs,
		       pa, pa_stddev, nsigma, add_chans,
		       plot_add_chans);	  

    else if (window_offset)
      do_window_offset(data,x1,x2,display, goodchans, freqs,
		       correct, pa, pa_stddev);
...
unsigned find_binmax ( const Iterator& begin, const Iterator& end,
		       T& max )
{
  Iterator it = std::max (begin, end);
  max = *it;
  return it - begin;
}
		 ...
		 vector<double>& pa, vector<double>& pa_stddev)
{
  ...
  int win = abs(bin1 - bin2);
  
  Reference::To<Pulsar::Archive> copy = data->clone();
  copy->fscrunch();
    
  //vector<double> offsets = get_offsets(copy);
  
  if (verbose) {
    cerr << "nbin = " << nbin << endl;
    cerr << "start bin = " << bin1 << endl;
    cerr << "end bin = " << bin2 << endl;
    cerr << "window = " << win << " bins" << endl;
  }
  
  for (int i = 0; i < good_nchan; i++) {
    const float* qchan = data->get_Profile(0,1,goodchans[i])->get_amps();
    const float* uchan = data->get_Profile(0,2,goodchans[i])->get_amps();
    
    vector<double> per_bin(win);
    ...
    if (verbose && display)
    {
      ...
      cpgswin (bin1, bin2, -180.0, 180.0);
      cpgbox("BCINTS", 0.0, 0, "BCINTS", 0.0, 0);    
      cpglab("Bin", "Angle (Degrees)", 
	     "Rotated PA in each bin");
      cpgslw(1);
      cpgsci(1);
      cpgsch(1.0);
    }
    ...
    for (int j = 0; j < win; j++)
    {
      int ibin = (bin1 + j)%nbin;
      per_bin[j] = 90.0 + 90.0/M_PI * atan2 (uchan[ibin], qchan[ibin]);
    ...
    if (correct) {
      if (per_bin[0] > 100.0) 
	cerr << "WARNING! Check first bin in channel " << i << endl;
      else {
	for (int j = 0; j < win-1; j++) {
	  if (abs(per_bin[j+1] - per_bin[j]) > 100) {
	    cerr << "Correcting bin #" << j+1 
		 << " in channel " << i << endl;
	    cerr << "   value was: " << per_bin[j+1];
	    if (per_bin[j+1] > 0.0)
	      per_bin[j+1] -= 180.0;
	    else
	      per_bin[j+1] += 180.0;
	    cerr << "   value is now: " << per_bin[j+1] << endl;
	  }
	}
      }
    }
    ...
    pa[i] = mean(per_bin);
    pa_stddev[i] = get_stddev(win,pa[i], per_bin);
  ...
  if( display )
    do_display(freqs, pa, pa_stddev,data->get_filename());
}
...
get_offsets(Reference::To<Pulsar::Archive> copy)
{
  vector<double> offsets(copy->get_nbin());  
  
  const float* q = copy->get_Profile(0,1,0)->get_amps();
  const float* u = copy->get_Profile(0,2,0)->get_amps();
  
  for (unsigned i = 0; i < offsets.size(); i++)
    offsets[i] = 90.0 + 90.0/M_PI * atan2 (u[i], q[i]);
...
get_per_bin(int rise_bin,int fall_bin,
	    ...
	    unsigned ichan)
{
  int on_pulse = abs(fall_bin - rise_bin);
  int nbin = data->get_nbin();

  vector<double> per_bin(on_pulse);

  const float* qchan = data->get_Profile(0,1,ichan)->get_amps();
  const float* uchan = data->get_Profile(0,2,ichan)->get_amps();   
  
  for (int j = 0; j < on_pulse; j++) {
    int ibin = (rise_bin + j)%nbin;
    per_bin[j] = 90.0 + 90.0/M_PI * atan2 (uchan[ibin], qchan[ibin]);
    //	per_bin[j] -= offsets[ibin];
  }
	   ...
	   const vector<double>& per_bin)
{
  ...
  for (int j = 0; j < on_pulse; j++)
    stddev += SQR(per_bin[j] - mean);
  
  if( on_pulse == 1 )
    stddev = 1.0;
  else
    stddev = sqrt(stddev/(on_pulse - 1.0));   
	   ...
	   string filename)
{
  ...
  minmax (pa, ymin, ymax);
  double errmax = *std::max (pa_stddev.begin(), pa_stddev.end());
  ...
  cpgswin (flo - 0.05*frange, fhi + 0.05*frange, ymin - errmax, ymax + errmax);
  cpgsch(2.0);
  cpgbox("BCINTS", 0.0, 0, "BCINTS", 0.0, 0);    
  cpglab("Frequency (MHz)", "Angle (Degrees)", filename.c_str());
	 ...
	 const vector<vector<unsigned> >& plot_add_chans)
{
  ...
  for (int i = 0; i < good_nchan; i++) {	  
    ...
    pa[i] = 90.0 + 90.0/M_PI * atan2 (tot_u, tot_q);    
    
    // Hope no wraps are involved:
    {
      float uerr = nsigma * sqrt(variance(uchan,uchan+nbin)* on_pulse);
      float qerr = nsigma * sqrt(variance(qchan,qchan+nbin)* on_pulse);

      //          pa = 90 + (90/pi) arctan(u/q)
      // differentiating and taking absolute values for terms in u and q and just wrongly adding them (no quadrature) gives:
      //    delta pa = 90/pi * 1/(1+u^2/q^2) * (delta u/|q| + |u| delta q/q^2)
      //      float term1 = 90.0 / M_PI;
      //float term2 = 1.0 / (1.0 + SQR(tot_u/tot_q));
      //float term3 = uerr/fabs(tot_q) + fabs(tot_u) * qerr/SQR(tot_q);
      
      //      pa_stddev[i] = term1 * term2 * term3;

      /*      if( verbose )
	      fprintf(stderr,"u=%f uerr=%f q=%f qerr=%f so term1=%f term2=%f term3=%f (term3a=%f term3b=%f) pa_stddev=%f*sqrt(%f)=%f\n",
	      tot_u,uerr,tot_q,qerr,
	      term1, term2, term3,
	      uerr/fabs(tot_q), fabs(tot_u) * qerr/SQR(tot_q),
	      term1 * term2 * term3, float(on_pulse),
	      pa_stddev[i]);
      */

      float a1 = 90.0 + 90.0/M_PI * atan2( tot_u+uerr, tot_q+qerr);
      float a2 = 90.0 + 90.0/M_PI * atan2( tot_u+uerr, tot_q-qerr);
      float a3 = 90.0 + 90.0/M_PI * atan2( tot_u-uerr, tot_q+qerr);
      float a4 = 90.0 + 90.0/M_PI * atan2( tot_u-uerr, tot_q-qerr);
      float maxa = max( max(a1,a2), max(a3,a4) );
      float mina = min( min(a1,a2), min(a3,a4) );
      
      double erra = max( fabs(maxa-pa[i]), fabs(mina-pa[i]) );
      ...
      if( a4 < 90.0 )	a4 += 180.0;
      double the_pa = pa[i];
      if( pa[i] < 90.0 ) the_pa += 180.0;

      maxa = max( max(a1,a2), max(a3,a4) );
      mina = min( min(a1,a2), min(a3,a4) );
            
      double errb = max( fabs(maxa-the_pa), fabs(mina-the_pa) );

      pa_stddev[i] = min(erra,errb);
      
      float l = sqrt(SQR(tot_u) + SQR(tot_q));
      float lerr = (fabs(tot_u*uerr)+fabs(tot_q*qerr))/l;

      if( verbose ){
	fprintf(stderr,"u=%f uerr=%f (%f) q=%f qerr=%f (%f) l=%f lerr=%f (%f)\n",
		tot_u,uerr,uerr/tot_u,tot_q,qerr,qerr/tot_q,
		l,lerr,lerr/l);
	fprintf(stderr,"a1=%f a2=%f a3=%f a4=%f\n",a1,a2,a3,a4);
	fprintf(stderr,"pa[i]=%f maxa=%f mina=%f erra=%f errb=%f\n",pa[i],maxa,mina,erra,errb);
      }
      
    }

    //vector<double> per_bin = get_per_bin(rise_bin,fall_bin, data, goodchans[i]);
    //pa[i] = findmean( &per_bin[0], &per_bin[0]+on_pulse);
    //pa_stddev[i] = get_stddev(on_pulse,pa[i], per_bin);

    if( is_one_of(unsigned(goodchans[i]),add_chans) ){
      pa[i] += 180.0;
    }
    if( iplot < plot_add_chans.size() && is_one_of(unsigned(goodchans[i]),plot_add_chans[iplot]) ){
      //      fprintf(stderr,"iplot=%d ichan=%d is one of plot_add_chans\n",iplot,goodchans[i]);
      pa[i] += 180.0;
    }

    if( verbose )
      fprintf(stderr,"i=%d (%d) of %d freq=%f pa=%f pa_stddev=%f\n\n",
	      i,goodchans[i], good_nchan, freqs[i], pa[i], pa_stddev[i]);
  }

...
void
Pulsar::IntegrationBarycentre::set_Integration (const Integration *_subint)
{
  if (subint == _subint)
    return;
  ...
  subint = _subint;
  const Archive *arch = 
    const_cast<Integration*>(subint.get())->expert()->get_parent();

  bary.set_epoch(subint->get_epoch());
  bary.set_coordinates(arch->get_coordinates());

  double x, y, z;
  const Site *site = Site::location(arch->get_telescope());
  site->get_xyz(x,y,z);

...
[
  ...
  if test "$have_epsic" != "user disabled"; then
    ...
    SWIN_PACKAGE_FIND([epsic],[mode.h])
    SWIN_PACKAGE_TRY_COMPILE([epsic],[#include "mode.h"])

    SWIN_PACKAGE_FIND([epsic],[libepsic.*])
    SWIN_PACKAGE_TRY_LINK([epsic],[#include "mode.h"],
                          [mode a ();],[-lepsic])
  ...
  if test $have_epsic = yes; then
    ...
    EPSIC_CFLAGS="$epsic_CFLAGS $EPSIC_CFLAGS"
  fi
